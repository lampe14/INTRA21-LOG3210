...
public class EstheRustConvertorVisitor implements ParserVisitor {
    ...
    public Object visit(ASTImportStmt node, Object data) {
        for (int i = 0; i < node.jjtGetNumChildren(); ++i) {
            ...
            node.jjtGetChild(i).jjtAccept(this, data); //noeud Identifier() (nom de librairie)
            m_writer.println("");
        }
        return null;
    }
    ...
    public Object visit(ASTDeclareStmt node, Object data) {
        ...
        node.jjtGetChild(0).jjtAccept(this, data); //noeud Identifier() (nom de variable)
        m_writer.print(" = ");
        node.jjtGetChild(1).jjtAccept(this, data); //noeud d'assignation (ASTAssignStmt)
        m_writer.println("");
        return null;
    }
    ...
    public Object visit(ASTAssignStmt node, Object data) {
        node.jjtGetChild(0).jjtAccept(this, data); //noeud Expr()
        return null;
    }
    ...
    public Object visit(ASTCompExpr node, Object data) {
        node.jjtGetChild(0).jjtAccept(this, data); //noeud AddExpr()
        if(node.getValue() != null)
            m_writer.print(" " + getOp(node.getValue()) + " ");
        if(node.jjtGetNumChildren() > 1)
            node.jjtGetChild(1).jjtAccept(this, data); //noeud CompExpr()
        return null;
    }

    @Override
    public Object visit(ASTAddExpr node, Object data) {
        node.jjtGetChild(0).jjtAccept(this, data); //noeud UnaExpr()
        if(node.getValue() != null)
            m_writer.print(" " + node.getValue() + " ");
        if(node.jjtGetNumChildren() > 1)
            node.jjtGetChild(1).jjtAccept(this, data); //noeud AddExpr()
        return null;
    }
    ...
    public Object visit(ASTUnaExpr node, Object data) {
        ...
        node.jjtGetChild(0).jjtAccept(this, data); //noeud UnaExpr()
        if(node.jjtGetNumChildren() > 1)
            node.jjtGetChild(1).jjtAccept(this, data); //noeud ParExpr()
    ...
    public Object visit(ASTParExpr node, Object data) {
        if(node.jjtGetChild(0) instanceof ASTExpr) {
            ...
            node.jjtGetChild(0).jjtAccept(this, data); //noeud PrimitiveExpr()
    ...
    private void printParam(SimpleNode node, Object data) {
        node.jjtGetChild(0).jjtAccept(this, data); //noeud Identifier() (nom de fct)
        m_writer.print("(");
        for(int i = 1; i < node.jjtGetNumChildren(); i++) {
            node.jjtGetChild(i).jjtAccept(this, data); //noeud Identifier() (parametre)
    ...
    public Object visit(ASTFunctionStmt node, Object data) {
        node.jjtGetChild(0).jjtAccept(this, data); //noeud FunctionParam()
        indentationLevel++;
        if(node.jjtGetChild(1).jjtGetNumChildren() == 0) //si noeud Block() est vide
            m_writer.println(INDENT + "pass");
        else
            node.jjtGetChild(1).jjtAccept(this, data); //imprime noeud Block()
        if(node.jjtGetNumChildren() == 3) { //s'il y a valeur de retour
            m_writer.print(INDENT + "return ");
            node.jjtGetChild(2).jjtAccept(this, data); //noeud Expr()
            m_writer.println("");
        }
        indentationLevel--;
        m_writer.println("\n");
    ...
    public Object visit(ASTCallFunction node, Object data) {
        ...
        printParam(node, data);
        m_writer.print(")");
        if(node.jjtGetParent() instanceof ASTStmt) //deja un saut de ligne si on l'appel de declarationStmt
            m_writer.println("");
        return null;
    }
    ...
    private void printBlock(SimpleNode node, int child, Object data) {
        ...
        if(node.jjtGetChild(child).jjtGetNumChildren() == 0) {
            ...
            node.jjtGetChild(child).jjtAccept(this,data); //noeud Block() (body)
        indentationLevel--;
    }
    ...
    public Object visit(ASTIfStmt node, Object data) {
        for(int i = 0; i < indentationLevel; i++)
            m_writer.print(INDENT);
        m_writer.print("if ");
        node.jjtGetChild(0).jjtAccept(this,data); //noeud Expr() (condition)
        printBlock(node, 1, data); //body
